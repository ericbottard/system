...
import (
	...
	"github.com/projectriff/system/pkg/reconciler/v1alpha1/streamingprocessor/resources"
	resourcenames "github.com/projectriff/system/pkg/reconciler/v1alpha1/streamingprocessor/resources/names"
	"go.uber.org/zap"
...
func NewController(
	...
	deploymentInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: controller.Filter(streamingv1alpha1.SchemeGroupVersion.WithKind("Processor")),
	...
	streamingInformer.Informer().AddEventHandler(reconciler.Handler(
		// Call the tracker's OnChanged method, but we've seen the objects
		// coming through this path missing TypeMeta, so ensure it is properly
		// populated.
		controller.EnsureTypeMeta(
			c.tracker.OnChanged,
			streamingv1alpha1.SchemeGroupVersion.WithKind("Stream"),
		),
	))
...
func (c *Reconciler) reconcile(ctx context.Context, processor *streamingv1alpha1.Processor) error {
	logger := logging.FromContext(ctx)
	...
	for _, inputName := range processor.Spec.Inputs {
		...
		gvk := streamingv1alpha1.SchemeGroupVersion.WithKind("Stream")
...
func (c *Reconciler) updateStatus(desired *streamingv1alpha1.Processor) (*streamingv1alpha1.Processor, error) {
	...
	p, err := c.ProjectriffClientSet.StreamingV1alpha1().Processors(desired.Namespace).UpdateStatus(existing)
...
func (c *Reconciler) reconcileDeployment(ctx context.Context, processor *streamingv1alpha1.Processor, deployment *appsv1.Deployment) (*appsv1.Deployment, error) {
	logger := logging.FromContext(ctx)
...
func (c *Reconciler) createDeployment(processor *streamingv1alpha1.Processor) (*appsv1.Deployment, error) {
